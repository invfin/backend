...
class TestNotificationSystem(TestCase):
    ...
    def setUpTestData(cls) -> None:
        cls.question = baker.make(Question, author=baker.make(User))
        cls.question_comment = baker.make(
            QuesitonComment,
            content_related=baker.make(Question),
            author=baker.make(User)
        )
        cls.answer = baker.make(Answer, author=baker.make(User))
        cls.answer_comment = baker.make(
            AnswerComment,
            author=baker.make(User),
            content_related=baker.make(Answer),
        )
        cls.writter = baker.make(User, is_writter=True)
        cls.followers = baker.make(NewsletterFollowers, user=cls.writter)
        cls.blog = baker.make(PublicBlog, author=cls.writter)
        cls.user_1 = baker.make(User)
        cls.user_2 = baker.make(User)
