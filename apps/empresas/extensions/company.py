...
class CompanyExtended(ChartSerializer):
    ...
    def balance_json(self, limit):
        ...
        bls_json = {
            "currency": currency,
            "labels": [data.date_year for data in bls],
            "fields": [
                {
    ...
    def rentability_ratios_json(self, limit):
        ...
        rr_json = {
            "labels": [data.date_year for data in rr],
            "fields": [
                {"title": "ROA", "url": "#!", "percent": "true", "short": "true", "values": [data.roa for data in rr]},
                {"title": "ROE", "url": "#!", "percent": "true", "short": "true", "values": [data.roe for data in rr]},
                {"title": "ROC", "url": "#!", "percent": "true", "short": "true", "values": [data.roc for data in rr]},
                {
                    "title": "ROCE",
                    "url": "#!",
                    "percent": "true",
    ...
    def liquidity_ratios_json(self, limit):
        ...
        lr_json = {
            "labels": [data.date_year for data in lr],
            "fields": [
                {
                    "title": "Cash Ratio",
                    "url": "#!",
    ...
    def non_gaap_json(self, limit):
        ...
        nongaap_json = {
            "labels": [data.date_year for data in nongaap],
            "fields": [
                {
    ...
    def calculate_averages(
        ...
        average_cash_flow_coverage_ratios = all_operation_risks_ratios.aggregate(
        ...
        average_debt_service_coverage = all_operation_risks_ratios.aggregate(
            average_debt_service_coverage=Avg("debt_service_coverage")
        )
        average_interest_coverage = all_operation_risks_ratios.aggregate(
            average_interest_coverage=Avg("interest_coverage")
        )
        average_operating_cashflow_ratio = all_operation_risks_ratios.aggregate(
        ...
        average_long_term_debt_to_capitalization = all_operation_risks_ratios.aggregate(
            average_long_term_debt_to_capitalization=Avg("long_term_debt_to_capitalization")
        )
        average_total_debt_to_capitalization = all_operation_risks_ratios.aggregate(
    ...
    def calculate_current_ratios(
        ...
        all_operation_risks_ratios = (
            all_operation_risks_ratios if all_operation_risks_ratios else self.all_operation_risks_ratios(10)
        )
        ...
        try:
            sharesbuyback = abs(
                (
                    (
                        (average_shares_out / all_inc_statements[number].weighted_average_shares_outstanding)
                        ** ((1 / num_ics))
                    )
                    - 1
                )
