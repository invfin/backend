class CalculateCompanyFinancialRatios:
    def generate_current_data(self, income_statements: list, balance_sheets: list, cashflow_statements: list) -> dict:
        current_data = {}
    ...
    def generate_last_year_data(self, income_statements: list, balance_sheets: list, cashflow_statements: list) -> dict:
        ly_data = {}
    ...
    def calculate_all_ratios(self, income_statements: list, balance_sheets: list, cashflow_statements: list) -> dict:
    ...
    def calculate_main_ratios(self, data: dict) -> dict:
        ...
        grossInvestedCapital = (
            netWorkingCapital + data["propertyPlantEquipmentNet"] + data["depreciationAndAmortization"]
        )
        ...
        nopat = (
            data["operatingIncome"] * (1 - (data["incomeTaxExpense"] / data["operatingIncome"]))
    ...
    def calculate_rentability_ratios(self, data: dict) -> dict:
        ...
        roic = (
            ((data["netIncome"] - data["dividendsPaid"]) / data["investedCapital"]) * 100
    ...
    def calculate_liquidity_ratio(self, data: dict) -> dict:
        cash_ratio = (
            data["cashAndCashEquivalents"] / data["totalCurrentLiabilities"]
            if data["totalCurrentLiabilities"] != 0
            else 0
        )
        current_ratio = (
            data["totalCurrentAssets"] / data["totalCurrentLiabilities"] if data["totalCurrentLiabilities"] != 0 else 0
        )
        quick_ratio = (
            (data["netReceivables"] + data["cashAndShortTermInvestments"]) / data["totalCurrentLiabilities"]
            if data["totalCurrentLiabilities"] != 0
            else 0
        )
        operating_cashflow_ratio = (
            data["netCashProvidedByOperatingActivities"] / data["totalCurrentLiabilities"]
            if data["totalCurrentLiabilities"] != 0
            else 0
        )
        debt_to_equity = (
            data["totalLiabilities"] / data["totalStockholdersEquity"] if data["totalStockholdersEquity"] != 0 else 0
        )
    ...
    def calculate_margin_ratio(self, data: dict) -> dict:
        ...
        unlevered_fcf_ebit_to_net_income = (
            data["unlevered_fcf_ebit"] / data["netIncome"] * 100 if data["netIncome"] != 0 else 0
        )
        owners_earnings_to_net_income = (
            data["owners_earnings"] / data["netIncome"] * 100 if data["netIncome"] != 0 else 0
        )
    ...
    def calculate_ps_value(self, data: dict) -> dict:
        ...
        book_ps = (
            data["totalStockholdersEquity"] / data["weightedAverageShsOut"] if data["weightedAverageShsOut"] != 0 else 0
        )
        tangible_ps = (
            data["netTangibleEquity"] / data["weightedAverageShsOut"] if data["weightedAverageShsOut"] != 0 else 0
        )
        ...
        cash_ps = (
            data["cashAndShortTermInvestments"] / data["weightedAverageShsOut"]
            if data["weightedAverageShsOut"] != 0
            else 0
        )
        operating_cf_ps = (
            data["netCashProvidedByOperatingActivities"] / data["weightedAverageShsOut"]
            if data["weightedAverageShsOut"] != 0
            else 0
        )
        capex_ps = (
            data["capitalExpenditure"] / data["weightedAverageShsOut"] if data["weightedAverageShsOut"] != 0 else 0
        )
        total_assets_ps = (
            data["totalAssets"] / data["weightedAverageShsOut"] if data["weightedAverageShsOut"] != 0 else 0
        )
    ...
    def calculate_non_gaap(self, data: dict) -> dict:
        ...
        income_quality = (
            (data["netCashProvidedByOperatingActivities"] / data["netIncome"]) * 100 if data["netIncome"] != 0 else 0
        )
        invested_capital = (
            data["propertyPlantEquipmentNet"] + data["netWorkingCapital"] - data["cashAndCashEquivalents"]
        )
        market_cap = data["currentPrice"] * data["weightedAverageShsOut"]
        net_current_asset_value = (
            (data["totalCurrentAssets"] - (data["totalLiabilities"])) / data["weightedAverageShsOut"]
            if data["weightedAverageShsOut"] != 0
            else 0
        )
    ...
    def calculate_operation_risk_ratio(self, data: dict) -> dict:
        ...
        cash_flow_coverage_ratios = (
            data["netCashProvidedByOperatingActivities"] / data["totalDebt"] if data["totalDebt"] != 0 else 0
        )
        cash_coverage = (
    ...
    def calculate_enterprise_value_ratio(self, data: dict) -> dict:
        ...
        company_equity_multiplier = (
            data["totalAssets"] / data["totalStockholdersEquity"] if data["totalStockholdersEquity"] != 0 else 0
        )
    ...
    def calculate_company_growth(self, data: dict) -> dict:
        ...
        cost_revenue_growth = (
            (data["costOfRevenue"] - data["last_year_cost_revenue"]) / data["last_year_cost_revenue"]
        ...
        net_income_growth = (
            (data["netIncome"] - data["last_year_net_income"]) / data["last_year_net_income"]
            if data["last_year_net_income"] != 0
            else 0
        )
        shares_buyback = (
            (data["weightedAverageShsOut"] - data["last_year_shares_outstanding"])
        ...
        fcf_growth = (
            (data["freeCashFlow"] - data["last_year_fcf"]) / data["last_year_fcf"] if data["last_year_fcf"] != 0 else 0
        )
        ...
        capex_growth = (
            (data["capitalExpenditure"] - data["last_year_capex"]) / data["last_year_capex"]
            if data["last_year_capex"] != 0
            else 0
        )
        rd_expenses_growth = (
            (data["researchAndDevelopmentExpenses"] - data["last_year_research_dev"]) / data["last_year_research_dev"]
    ...
    def calculate_eficiency_ratio(self, data: dict) -> dict:
        days_inventory_outstanding = (
            data["average_inventory"] / (data["costOfRevenue"] * 360) if data["costOfRevenue"] != 0 else 0
        )
        days_payables_outstanding = (
            (data["accountPayables"] * 360) / data["costOfRevenue"] if data["costOfRevenue"] != 0 else 0
        )
        days_sales_outstanding = (
            (data["accountsReceivables"] * 360) / data["accountPayables"] if data["accountPayables"] != 0 else 0
        )
        ...
        fcf_to_operating_cf = (
            data["freeCashFlow"] / data["netCashProvidedByOperatingActivities"]
            if data["netCashProvidedByOperatingActivities"] != 0
            else 0
        )
