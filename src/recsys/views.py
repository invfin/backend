...
class RecommendationClickedRedirectView(RedirectView):
    ...
    def generate_url(self, obj: Model, medium: str = "webapp", source: str = "invfin", campaign: str = "recsys"):
        ...
        return f"?{utm_source}&{utm_medium}&{utm_content}&{utm_campaign}&{utm_term}&{place}&{style}"
...
class BaseRecommendationView(TemplateView):
    ...
    def create_recommendations(
        self, recommendations: Union[QuerySet, List], recommendation_explained: Dict = None
    ) -> List:
        """
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context.update({"recommendations": self.return_recommendations()})
        return context
...
class BaseCompanyVisitedRecommendationView(BaseRecommendationView):
    ...
    def get_specific_company_recommendations(self, companies_visited: List) -> QuerySet:
    ...
    def generate_recommendations_explanations(self) -> Union[Dict, List]:
        ...
        explanations
        """
        return {"reason": "most_total_visits", "sliced": True, "slice_size": 5}
