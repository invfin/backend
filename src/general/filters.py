...
class JSONFieldFilter(SimpleListFilter):
    ...
    def __init__(self, request, params, model, model_admin):
        ...
        assert hasattr(self, "title"), 'Class {} missing "title" attribute'.format(
    ...
    def lookups(self, request, model_admin):
        ...
        """
        if (
            "__" in self.json_field_property_name
        ):  # NOTE: this will cover only one nested level
    ...
    def queryset(self, request, queryset):
        if self.value():
            json_field_query = {
                "{}__{}".format(
                    self.json_field_name, self.json_field_property_name
                ): self.value()
            }
