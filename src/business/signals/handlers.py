...
from src.business.stripe_management import StripeManagement
from src.users import constants as credits_constants
    ...
    def product_pre_save(cls, sender, instance: Product, **kwargs):
        if not instance.pk:
            ...
            stripe_product = cls.stripe.update_product(
                instance.stripe_id, instance.title, instance.description, instance.is_active
            )
    ...
    def product_complementary_pre_save(cls, sender, instance: Product, **kwargs):
        ...
        if not instance.pk:
            ...
            stripe_product_complementary = cls.stripe.update_product_complementary(
                instance.stripe_id, instance.is_active
            )
        instance.updated_at = timezone.now()
    ...
    def transaction_post_save(cls, sender, instance: TransactionHistorial, **kwargs):
        ...
        # Create something to invite the user to purchase a subscription to get unlimited credits
            ...
            subscription = ProductSubscriber.objects.create(
                product=instance.product, product_complementary=instance.product_complementary, subscriber=user
            )
            Key.objects.create(user=user, subscription=subscription)
