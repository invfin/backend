...
class ProductDetailView(SEODetailView):
    ...
    def get_object(self, *args):
        return self.model._default_manager.get(slug=self.kwargs["slug"], is_active=True, for_testing=IS_TESTING,)
    ...
    def get(self, request, *args, **kwargs) -> HttpResponse:
        ...
        try:
            checkout_session = stripe.checkout.Session.create(
                ...
                success_url=f"{settings.FULL_DOMAIN}{success_path}?prod={product.id}" + "&session_id={CHECKOUT_SESSION_ID}",
...
class CheckoutRedirectView(RedirectView):
    ...
    def handle_messages(self):
        if self.handle_transaction():
            messages.success(self.request, "Gracias por tu confianza.",)
        else:
            messages.error(self.request, "Lo lamentamos ha sucedido un error. Lo estamos resolviendo",)

    def handle_transaction(self) -> bool:
        try:
            product_complementary = ProductComplementary.objects.get(pk=self.request.GET.get("prod"),)
    ...
    def get_customer(self, stripe_session:stripe.checkout.Session, stripe_customer:stripe.Customer,)->Customer:
        user = User.objects.get_or_create_quick_user(self.request, stripe_customer["email"],)
        customer, created = Customer.objects.get_or_create(user=user, defaults={"stripe_id": stripe_session.customer,},)
        return customer

    def save_transaction(self, stripe_response:stripe.checkout.Session, customer:Customer, product_complementary:ProductComplementary,)->None:
