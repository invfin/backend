...
from apps.empresas.models import Company, Exchange, ExchangeOrganisation, IncomeStatement
...
class TestCompanyManagers:
    @classmethod
    def setup_class(cls):
        cls.sector = DTM.create(Sector, sector="sector")
        cls.industry = DTM.create(Industry, industry="industry")
        cls.fr_main = DTM.create(ExchangeOrganisation, name="France")
        cls.usa_main = DTM.create(ExchangeOrganisation, name="Estados Unidos")
        cls.nyse = DTM.create(Exchange, exchange_ticker="NYSE", main_org=cls.usa_main)
        cls.euro = DTM.create(Exchange, exchange_ticker="EURO", main_org=cls.fr_main)
    ...
    def test_filter_checkings(self):
        ...
        assert [self.apple, self.google, self.intel] == list(Company.objects.filter_checkings("institutionals", False))
